...
class RoiAdjuster {
  ...
  handleMouseDown(e) {
    ...
    // Tính tỉ lệ co giãn giữa kích thước hiển thị và độ phân giải thực của canvas
    const scaleX = this.canvas.width / rect.width;
    const scaleY = this.canvas.height / rect.height;

    // Lấy tọa độ click chuột so với canvas và nhân với tỉ lệ co giãn
    const x = (e.clientX - rect.left) * scaleX;
    const y = (e.clientY - rect.top) * scaleY;

    // --- KẾT THÚC PHẦN SỬA LỖI ---

    // Xử lý vẽ vùng ROI (phần này giữ nguyên)
    if (this.isDrawingWaiting) {
        this.waitingZone.push([x, y]);
    } else if (this.isDrawingViolation) {
        this.violationZone.push([x, y]);
