    ...
    def recognize(self, vehicle_roi) -> Tuple[Optional[str], float]:
        ...
        with torch.no_grad():
            if self.plate_detector is None:
                ...
                plate_detections = self.plate_detector(processed_roi, device=self.device, verbose=False)
            else:
                plate_detections = self.plate_detector(processed_roi, device='cpu', verbose=False)
        ...
        # BƯỚC 3: DÙNG EASYOCR ĐỂ ĐỌC KÝ TỰ TỪ BIỂN SỐ
        try:
            ocr_results = self.reader.readtext(plate_crop)

            if not ocr_results:
                logger.warning("EasyOCR không phát hiện được text")
                return None, 0.0

            # Ghép các ký tự và lấy độ tin cậy trung bình
            full_text = ""
            ...
            final_text = self._clean_plate_text(full_text)
            _ = total_confidence / len(ocr_results)  # OCR confidence for potential future use

            if not final_text:
                logger.warning("Text sau khi làm sạch bị rỗng")
                return None, 0.0

            # BƯỚC 4: Hậu xử lý thông minh để sửa lỗi và định dạng
            formatted_plate, score = self._post_process(final_text)

            if formatted_plate:
                logger.info(f"✓ Phát hiện biển số: '{formatted_plate}' với độ tin cậy: {score:.2f}")
                return formatted_plate, score
            else:
                logger.warning("Hậu xử lý không thành công")
                return None, 0.0
